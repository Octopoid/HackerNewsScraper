namespace HackerNewsScraper.Helpers
{
    ...
    {
        /// <summary>
        ///     Takes a given uri and attempts to make it valid. Absolute uris are returned as are, relative uris are prepended
        ///     with the fallback Uri, and invalid uris are replaced with the fallback uri.
        /// </summary>
        /// <param name="uri">The uri which must be absolute.</param>
        /// <param name="fallbackUri">
        ///     The fallback Uri to use for invalid Uris. This value is also used as the base domain for
        ///     relative uris.
        /// </param>
        /// <returns></returns>
        public static string EnsureUriAbsolute(string uri, string fallbackUri)
        {
            ...
            if (Uri.TryCreate(uri, UriKind.Relative, out Uri _))
            {
